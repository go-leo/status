...
func TestErrorInfo(t *testing.T) {
	...
	st := status.InvalidArgument(status.ErrorInfo(reason, domain, metadata))
...
func TestRetryInfo(t *testing.T) {
	...
	st := status.Unknown(status.RetryInfo(retryDelay))
...
func TestDebugInfo(t *testing.T) {
	...
	detail := "user exists"
	st := status.AlreadyExists(status.DebugInfo(stackEntries, detail))
...
func TestQuotaFailure(t *testing.T) {
	...
	st := status.ResourceExhausted(status.QuotaFailure(violations))
...
func TestPreconditionFailure(t *testing.T) {
	...
	st := status.FailedPrecondition(status.PreconditionFailure(violations))
...
func TestBadRequest(t *testing.T) {
	...
	st := status.InvalidArgument(status.BadRequest(violations))
...
func TestRequestInfo(t *testing.T) {
	requestId := ""
	servingData := ""
	st := status.PermissionDenied(status.RequestInfo(requestId, servingData))
...
func TestResourceInfo(t *testing.T) {
	...
	st := status.Aborted(status.ResourceInfo(resourceType, resourceName, owner, description))
...
func TestHelp(t *testing.T) {
	...
	st := status.OutOfRange(status.Help(links))
...
func TestLocalizedMessage(t *testing.T) {
	locale := "locale"
	message := "message"
	st := status.DataLoss(status.LocalizedMessage(locale, message))
...
func TestDetail(t *testing.T) {
	...
	st := status.Unimplemented(status.Detail(detail))
